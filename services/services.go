...
// Query query service
func (ctrl *ServiceCtrl) Query(ctx context.Context, clientIP net.IP, service string, proto bool, endpoints bool) (*ServiceV1, int64, error) {
	if err := checkService(service); err != nil {
		return nil, 0, err
	}
	if ctrl.ProtoSwitch {
		return ctrl._query(ctx, clientIP, service, proto, endpoints)
	}
...
func (ctrl *ServiceCtrl) QueryServiceZone(ctx context.Context, clientIP net.IP, service string, zone string, proto bool, endpoints bool) (*ServiceV1, int64, error) {
	key := ctrl.serviceZoneKey(service, zone)
	return ctrl._query(ctx, clientIP, key, proto, endpoints) // key 为 `service/zone`
}
...
func (ctrl *ServiceCtrl) _query(ctx context.Context, clientIP net.IP, serviceKey string, proto bool, endpoints bool) (*ServiceV1, int64, error) {
	...
	service, err := ctrl.makeService(ctx, clientIP, serviceKey, resp.Kvs, proto, endpoints)
	makeCostTime := time.Since(makeStartTime)
	if makeCostTime.Milliseconds() > 1000 {
		glog.Warningf("makeService Key: %s Cost: %s", key, makeCostTime)
	}
	quertCostTime := time.Since(queryStartTime)
	if quertCostTime.Milliseconds() > 1000 {
		glog.Warningf("queryService Key: %s Cost: %s", key, quertCostTime)
	}
	if err != nil {
		return nil, 0, err
	}
...
// Watch watch service
func (ctrl *ServiceCtrl) Watch(ctx context.Context, clientIP net.IP, serviceKey string, revision int64, proto bool, endpoints bool) (*ServiceV1, int64, error) {
	...
	_ = <-watchCh
	return ctrl._query(ctx, clientIP, serviceKey, proto, endpoints)
}
